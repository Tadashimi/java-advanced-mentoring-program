...
public class MyThreadSafeMapWithSynchronizationTest {
    ...
    public void getTest() throws InterruptedException {
        List<MyThreadSafeMapWithSynchronization.MyEntry<Integer, Integer>> entries = new ArrayList<>();

        for (int i = 0; i < 5; i++) {
            entries.add(new MyThreadSafeMapWithSynchronization.MyEntry<>(random.nextInt(KEYS_BOUNDARY), random.nextInt()));
        }
        MyThreadSafeMapWithSynchronization<Integer, Integer> map = new MyThreadSafeMapWithSynchronization<>(entries);
    ...
    public void putTest() throws InterruptedException {
        MyThreadSafeMapWithSynchronization<Integer, Integer> map = new MyThreadSafeMapWithSynchronization<>();
        for (int i = 0; i < THREAD_COUNT; i++) {
    ...
    private static Runnable getReadRunnable(MyThreadSafeMapWithSynchronization<Integer, Integer> map) {
        return () -> {
    ...
    private static Runnable getWriteRunnable(MyThreadSafeMapWithSynchronization<Integer, Integer> map) {
        return () -> {
            for (int i = 0; i < NUMBER_OF_OPERATIONS; i++) {
    ...
    private static Runnable getRemoveRunnable(MyThreadSafeMapWithSynchronization<Integer, Integer> map) {
        return () -> {
            for (int i = 0; i < NUMBER_OF_OPERATIONS; i++) {
                Integer key = ThreadLocalRandom.current().nextInt(KEYS_BOUNDARY);
                map.remove(key);
    ...
    private static Runnable getKeysRunnable(MyThreadSafeMapWithSynchronization<Integer, Integer> map) {
        return () -> {
            for (int i = 0; i < NUMBER_OF_OPERATIONS; i++) {
    ...
    private static Runnable getValuesRunnable(MyThreadSafeMapWithSynchronization<Integer, Integer> map) {
        return () -> {
            for (int i = 0; i < NUMBER_OF_OPERATIONS; i++) {
    ...
    private static Runnable getIteratorRunnable(MyThreadSafeMapWithSynchronization<Integer, Integer> map) {
        return () -> {
            for (int i = 0; i < NUMBER_OF_OPERATIONS; i++) {
                Iterator<MyThreadSafeMapWithSynchronization.MyEntry<Integer, Integer>> iterator = map.iterator();
                if (iterator.hasNext()) {
